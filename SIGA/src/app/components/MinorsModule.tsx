...
const INITIAL_FORM: MinorForm = {
  nombre: "", apellido: "", rut: "", pasaporte: "", fecNac: "", docType: "notarial",
};

export function MinorsModule({ onNavigate, onLogout }: MinorsModuleProps) {
  const [form, setForm] = useState<MinorForm>(INITIAL_FORM);
  const [files, setFiles] = useState<File[]>([]);
  const [pdiStatus, setPdiStatus] = useState<PDIStatus>("idle");
  const [errors, setErrors] = useState<Partial<MinorForm>>({});

  const handleField = useCallback(
    (field: keyof MinorForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
  ...
  const validate = (): Partial<MinorForm> => {
    const e: Partial<MinorForm> = {};

    const nombreErr = validateName(form.nombre);
    if (nombreErr) e.nombre = nombreErr;

    const apellidoErr = validateName(form.apellido);
    if (apellidoErr) e.apellido = apellidoErr;

    if (!form.rut && !form.pasaporte) {
      e.rut = "RUT o Pasaporte requerido";
    } else if (form.rut) {
      const rutErr = validateRut(form.rut);
      if (rutErr) e.rut = rutErr;
    }

    if (!form.fecNac) e.fecNac = "Fecha de nacimiento requerida";
    return e;
  };
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 py-8">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 flex items-start gap-3">
          ...
          <p className="text-blue-800 text-sm">
            Los menores de 18 años que viajan sin ambos padres requieren autorización notarial
            o de juzgado para salir del país.
          </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-5">
          <h3
            className="text-sm text-foreground mb-4 pb-2 border-b border-border"
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-5">
          ...
          <button className="flex items-center gap-2 border border-border hover:bg-muted px-4 py-2 rounded-md text-sm transition-colors mt-3">
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          {pdiStatus === "idle" && (
            <button
              ...
              className="w-full bg-primary hover:bg-blue-900 text-white py-3 rounded-md text-sm transition-colors flex items-center justify-center gap-2"
            >
              ...
              <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <div>
                  ...
                  <p className="text-red-700 text-xs mt-0.5">
                    Se encontraron inconsistencias. Contacte al funcionario.
                  </p>
                </div>
              </div>
              <button
                onClick={() => setPdiStatus("idle")}
